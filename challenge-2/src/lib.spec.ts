...
describe("Extra tests", () => {
  ...
  it("should throw an error values are now valid dates", () => {
    // Arrange
    const time = "2021-01-10T01:30:00.000Z";
    ...
    // Assert
    expect(() =>
      formatEstimatedWaitTime("", lowerBoundEstimate, upperBoundEstimate)
    ).toThrow();
    expect(() =>
      formatEstimatedWaitTime(time, "", upperBoundEstimate)
    ).toThrow();
    expect(() =>
      formatEstimatedWaitTime(time, lowerBoundEstimate, "")
    ).toThrow();
  });
